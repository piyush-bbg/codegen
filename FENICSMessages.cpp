...
K OrderDeleted::to_k(vector<QMessage*>& v) const
{
  int size = (int) v.size();
  K x = qb_knk(
               ktn(KI, size),
  ...
  for (int i = 0; i < size; i++)
    {
      FENICS::OrderDeleted* q = (FENICS::OrderDeleted*)v[i];
      ...
      kJ(xK[5])[i] = KTIME(q->recvd_time);
    }
  return x;
}

void OrderDeleted::print() const
{
  LOG_INFO(tableName()<<": "
           << seq_num << ", "
           << symbol << ", "
...
ostream& OrderDeleted::operator >> (ostream& os)
{
  os<<"tablename: "<<tableName()<<", ";
  return os;
}

void OrderDeleted::release()
{
  clear();
  Core::ObjectStore::release(this);
}

bool QB::MktData::FENICS::put(QB::Core::IOPacket& p, const OrderDeleted& v)
{
  bool ret = true;
...
bool QB::MktData::FENICS::get(QB::Core::IOPacket& p, OrderDeleted& v)
{
  bool ret = true;
...
void OrderExecuted::clear()
{
  ...
  SET_NULL(filled_qty);
  SET_NULL(trade_id);
  SET_NULL(trade_price);
  SET_NULL(recvd_time);
}

K OrderExecuted::to_k(vector<QMessage*>& v) const
{
  int size = (int) v.size();
  K x = qb_knk(
               ktn(KI, size),
  ...
  for (int i = 0; i < size; i++)
    {
      FENICS::OrderExecuted* q = (FENICS::OrderExecuted*)v[i];
      ...
      kJ(xK[3])[i] = KTIME(q->source_time);
      kJ(xK[4])[i] = KLONG(q->order_id);
      kI(xK[5])[i] = KINT(q->filled_qty);
      kJ(xK[6])[i] = KLONG(q->trade_id);
      kF(xK[7])[i] = KDBL(q->trade_price);
      kJ(xK[8])[i] = KTIME(q->recvd_time);
    }
  return x;
}

void OrderExecuted::print() const
{
  LOG_INFO(tableName()<<": "
           << seq_num << ", "
           << symbol << ", "
...
ostream& OrderExecuted::operator >> (ostream& os)
{
  os<<"tablename: "<<tableName()<<", ";
  return os;
}

void OrderExecuted::release()
{
  clear();
  Core::ObjectStore::release(this);
}

bool QB::MktData::FENICS::put(QB::Core::IOPacket& p, const OrderExecuted& v)
{
  bool ret = true;
  ...
  ret&=put(p,v.source_time);
  ret&=put(p,v.order_id);
  ret&=put(p,v.filled_qty);
  ret&=put(p,v.trade_id);
  ret&=put(p,v.trade_price);
  ret&=put(p,v.recvd_time);

  return ret;
}

bool QB::MktData::FENICS::get(QB::Core::IOPacket& p, OrderExecuted& v)
{
  bool ret = true;
  ...
  ret&=get(p,v.source_time);
  ret&=get(p,v.order_id);
  ret&=get(p,v.filled_qty);
  ret&=get(p,v.trade_id);
  ret&=get(p,v.trade_price);
  ret&=get(p,v.recvd_time);
